...
const Pricelist = ({ navigation }) => {
    ...
    const pesan = async (id_wedding) => {
        let booking = { id_user: user.id_user, jenis_booking: 'Wedding', id_item: id_wedding, tanggal_booking: selectedDate, bukti_transfer: '', status: 1 }
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <ImageBackground source={ImageHeader} style={styles.header}>
                    <View style={styles.hello}>
                        <Text style={styles.selamat}>Wedding</Text>
                            ...
                            <View style={styles.container}>
                                <View style={styles.informasiSaldo2}>
                                    <View style={styles.text}>
                                        <Text style={styles.valueSaldo}>{wed.jenis_wedding}</Text>
                                        
                                    </View>
                                    <View style={styles.text}>
                                        <Image
                                            style={{ width: 150, margin: 10, height: 100 }}
                                            source={{ uri: wed.gambar_wedding }}
                                        ></Image>
                                    </View>
                                    <View style={styles.text}>
                                        <Text style={styles.labelPoint}>Fasilitas : {wed.fasilitas}</Text>
                                    </View>
                                    <Button
                                        title="Detail"
                                        onPress={() => {
                ...
                <Modal
                    ...
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            <Text style={styles.modalText}>Wedding</Text>
                            <Image
                                style={{ Align: 'center', width: 100, margin: 10, height: 150 }}
                                source={{ uri: wed[selectedIndex]?.gambar_wedding }}
                            />
                            <Text style={styles.valueSaldo}>{wed[selectedIndex]?.jenis_wedding}</Text>
                            <Text style={styles.labelPoint}>{wed[selectedIndex]?.fasilitas}</Text>
                            <Text style={styles.labelHarga}>Rp.{wed[selectedIndex]?.harga}</Text>
                            <DatePicker
                            ...
                            /> */}
                                ...
                                <Pressable
                                    style={[styles.button, styles.buttonClose]}
                                    onPress={() => pesan(wed[selectedIndex]?.id_wedding)}>
