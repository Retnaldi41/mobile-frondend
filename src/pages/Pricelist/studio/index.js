...
const Pricelist = ({ navigation }) => {
    ...
    const pesan = async (id_studio) => {
        let booking = { id_user: user.id_user, jenis_booking: 'Studio', id_item: id_studio, tanggal_booking: selectedDate, bukti_transfer: '', status: 1 }
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <ImageBackground source={ImageHeader} style={styles.header}>
                    <View style={styles.hello}>
                        <Text style={styles.selamat}>Studio</Text>
                                ...
                                <View style={styles.informasiSaldo2}>
                                    ...
                                    <View style={styles.text}>
                                        <Image
                                            ...
                                            source={{ uri: stu.gambar_studio }}
                                        ></Image>
                                    </View>
                                    <View style={styles.text}>
                                        <Text style={styles.labelPoint}>Fasilitas : {stu.fasilitas}</Text>
                                    </View>
                                    <Button
                                        title="Detail"
                                        onPress={() => {
                ...
                <Modal
                    ...
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            <Text style={styles.modalText}>Studio</Text>
                            <Image
                                style={{ Align: 'center', width: 100, margin: 10, height: 150 }}
                                source={{ uri: stu[selectedIndex]?.gambar_studio }}
                            />
                            <Text style={styles.valueSaldo}>{stu[selectedIndex]?.jenis_studio}</Text>
                            <Text style={styles.labelPoint}>{stu[selectedIndex]?.fasilitas}</Text>
                            <Text style={styles.labelHarga}>Rp.{stu[selectedIndex]?.harga}</Text>
                            <DatePicker
                            ...
                            /> */}
                                ...
                                <Pressable
                                    style={[styles.button, styles.buttonClose]}
                                    onPress={() => pesan(stu[selectedIndex]?.id_studio)}>
