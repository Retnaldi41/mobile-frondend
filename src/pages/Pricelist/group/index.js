...
const Pricelist = ({ navigation }) => {
    ...
    useEffect(() => {
        getGroup(); // eslint-disable-next-line
    }, []);

    async function getGroup() {
        await axios.get('http://192.168.1.10:3000/group/')
            .then((response) => {
    ...
    const pesan = async (id_group) => {
        let booking = { id_user: user.id_user, jenis_booking: 'Group', id_item: id_group, tanggal_booking: selectedDate, bukti_transfer: '', status: 1 }
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                <ImageBackground source={ImageHeader} style={styles.header}>
                    <View style={styles.hello}>
                        <Text style={styles.selamat}>group</Text>
                            ...
                            <View style={styles.container}>
                                <View style={styles.informasiSaldo2}>
                                    <View style={styles.text}>
                                        <Text style={styles.valueSaldo}>{cos.jenis_group}</Text>

                                    </View>
                                    <View style={styles.text}>
                                        <Image
                                            style={{ width: 150, margin: 10, height: 100 }}
                                            source={{ uri: cos.gambar_group }}
                                        ></Image>
                                    </View>
                                    <View style={styles.text}>
                    ...
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            ...
                            <Image
                                ...
                                source={{ uri: cos[selectedIndex]?.gambar_group }}
                            />
                            <Text style={styles.valueSaldo}>{cos[selectedIndex]?.jenis_group}</Text>
                            ...
                            /> */}
                                ...
                                <Pressable
                                    style={[styles.button, styles.buttonClose]}
                                    onPress={() => pesan(cos[selectedIndex]?.id_group)}>
